...
$wgKwidgetPsEnabledModulesToStudio = array();
    ...
    if (isset($plugins)){
        foreach ($plugins as $key => $value) {
            ...
            if(!empty($value['category'])){
                $wgKwidgetPsEnabledModulesToStudio[$value['category']] =array(
                    "children"=> array(
                        $key => ""
                    )
                );
...
//This one we don't need any more if we will use next code block
...
//    }
//}
//$wgKwidgetPsEnabledModulesToStudio dynamically generated by parsing ps/modules folders
if( !empty($wgKwidgetPsEnabledModulesToStudio) ){
    foreach ($wgKwidgetPsEnabledModulesToStudio as $menuItem => &$menuContent) {
        foreach ($menuContent['children'] as $pluginName => &$pluginData) {
            if (isset($configRegister[$pluginName]) && isset($configRegister[$pluginName]['attributes'])) {
                $pluginData = $menuMaker->Menu($pluginName, $configRegister[$pluginName]);
            }
        ...
        if (isset($menu[$menuItem])){
            $menu[$menuItem]['children'] = array_merge_recursive($menu[$menuItem]['children'], $wgKwidgetPsEnabledModulesToStudio[$menuItem]['children']);
        }
    }
}
