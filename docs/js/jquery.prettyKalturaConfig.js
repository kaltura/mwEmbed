...
(function( $ ){
	...
	kPrettyConfig.prototype = {
		...
		init: function( settings ){
			...
			var _this = this;
			// inhert settings:
			$.each( settings, function( sName, sVal ){
				_this[sName] = sVal;
			});
			// set the target to loading while documentation is loaded
			$( this.target ).html('Loading <span class="blink">...</span>');
			
			// build the list of basevars
			var baseVarsList = '';
			$.each( _this.flashvars, function( fvKey, fvValue ){
				baseVarsList+= fvKey + ',';
			})
			...
			request+= '&plugin_id=' +	_this.pluginName + '&vars=' + baseVarsList;
			...
			$.getJSON( request, function( data ){
				...
				_this.manifestData = data;
				// merge in player config values into manifestData
				_this.mergeManifestData();
				
				$textDesc = '';
				if( _this.manifestData[ _this.pluginName ] ){
					$textDesc = $('<div />');
					if( _this.manifestData[ _this.pluginName ]['description']  ){
						$textDesc.html( _this.manifestData[ _this.pluginName ]['description'] );
					} else if( _this.manifestData[ _this.pluginName ]['doc'] ){ // also check plugin attribute id
						$textDesc.html( _this.manifestData[ _this.pluginName ]['doc'] );
					}
				} 
				...
				var settingTabHtml = ( _this.showSettingsTab ) ? 
					...
					.append(
						...
						$('<div class="tab-content" />').append(
							$('<div class="tab-pane active" id="tab-desc-' + id + '" />').append( $textDesc ),
						 	$('<div class="tab-pane active" id="tab-edit-' + id + '" />').append( _this.getEditTabs() ),
		...
		getEditTabs: function(){
			// conditionally include liShare and liEmbed
			var	$liShare = $();
			var $liEmbed = $();
			if( _this.showSettingsTab ){
		...
		getInput: ( key ){
			...
			$settings.append(
				...
				.click(function(){
					...
					_this.flashvarCallback( _this.flashvars );
		...
		mergeManifestData: function(){
			var _this = this;
			$.each( _this.flashvars, function( fvKey, fvValue ){
				if( fvKey == _this.pluginName  ){
					for( var pk in fvValue ){
						if( ! _this.manifestData[ _this.pluginName ].attributes[ pk ] ){
							_this.manifestData[ _this.pluginName ].attributes[ pk ] = {};
						}
						_this.manifestData[ _this.pluginName ].attributes[ pk ].value = fvValue[pk];
					}
					// continue
					return true;
				}
				// Check for prefixed vars ( _this.pluginName.varKey )
				if( fvKey.indexOf( _this.pluginName ) === 0 ){ 
					var fvParts = fvKey.split('.');
					_this.manifestData[ _this.pluginName ].attributes[ fvParts[1] ] = fvValue;
					// continue
					return true;
				} 
				if( typeof fvValue == 'object' ){
					for( var pk in fvValue ){
						if( typeof _this.manifestData[ fvKey ] == 'undefined' ){
							_this.manifestData[ fvKey ] = {};
							_this.manifestData[ fvKey ].attributes = {};
						}
						if( typeof _this.manifestData[ fvKey ].attributes[ pk ] == 'undefined' ){
							_this.manifestData[ fvKey ].attributes[ pk ] = {};
						}
						_this.manifestData[ fvKey ].attributes[ pk ].value = fvValue[pk];
					}
				} else {
					if( !_this.manifestData[ fvKey ] ){
						_this.manifestData[ fvKey ] = {};
					}
					_this.manifestData[ fvKey ].value = fvValue;
				}
			});
		},
		/**
		 ...
		 */
		getVarObj: function( attrName ){
			var _this = this;
			// Check for plugin config: 
			if(  _this.manifestData[_this.pluginName] && _this.manifestData[_this.pluginName].attributes &&  
					_this.manifestData[_this.pluginName].attributes[ attrName ] )
			{
				return _this.manifestData[_this.pluginName].attributes[ attrName ];
			}
			
			// Check other plugins
			for( var pid in _this.manifestData ){
				if( _this.manifestData[ pid ] && _this.manifestData[ pid ].attributes &&
						_this.manifestData[ pid ].attributes[ attrName ]	)
				{
					return _this.manifestData[ pid ].attributes[ attrName ];
				}
			}
			
			// Check for raw value object: 
			if( _this.manifestData[attrName] ){
				return _this.manifestData[attrName]; 
			}
		...
		setAttrValue: function( attrName, attrValue ){
			...
			if( _this.manifestData[_this.pluginName] && _this.manifestData[_this.pluginName].attributes &&  
					_this.manifestData[_this.pluginName].attributes[ attrName ] ){
				_this.manifestData[_this.pluginName].attributes[ attrName ].value = attrValue;
				// refresh the value
				_this.manifestData[_this.pluginName].attributes[ attrName ].$editVal.getEditValue( attrName );
			} else if( _this.manifestData[attrName] ){
				_this.manifestData[attrName].value = attrValue;
				// refresh the value
				_this.manifestData[attrName].$editVal.getEditValue( attrName );
			} else {
				// look for other plugins with this property:
				for( var pid in _this.manifestData ){
					if( _this.manifestData[pid].attributes ){
						for( var pAttrName in _this.manifestData[pid].attributes ){
							if( pAttrName == attrName ){
								_this.manifestData[pid].attributes[ attrName ].value = attrValue;
								// refresh the value
								_this.manifestData[pid].attributes[ attrName ].$editVal.getEditValue( attrName );
							}
						}
					}
				}
		 ...
		 */
		getJsonQuoteValue: function( attrName ){
			var _this = this;
			var val = _this.getAttrValue( attrName )
		...
		getAttrValue: function( attrName ){
			var _this = this;
			var attrValue = ( typeof _this._this.getVarObj( attrName ).value != 'undefined' ) ? 
								_this.getVarObj( attrName ).value :
								null;
			if( attrValue === true )
				attrValue = 'true';
			if( attrValue === false )
		...
		getAttrType: function( attrName ){
			var _this = this;
			return _this.getVarObj( attrName )['type'] || 'string';
		},
		getAttrDesc: function( attrName ){
			var _this = this;
			if( _this.getVarObj(  attrName )[ 'doc' ] ){
				return _this.getVarObj(  attrName )[ 'doc' ];
			}
		},
		 ...
		 */
		getConfiguredFlashvars: function(){
			var _this = this;
			var configuredFlashvars = $.extend( {}, _this.flashvars );
			$.each( _this.manifestData, function( pName, attr ){
				if( pName == _this.pluginName || attr.attributes ){
					$.each( _this.manifestData[pName].attributes, function( attrName, attr ){
						if( ! configuredFlashvars[ pName ] ){
							configuredFlashvars[ pName ] = {};
						}
						configuredFlashvars[ pName ] [ attrName ] = _this.getAttrValue( attrName );
					} )
				} else {
		...
		isDataError: function( data ){
			var _this = this;
			return ( !data || data.code );
		},
		addUiVarsToUiConf: function( confFile ){
			var _this = this;
			...
			// update /add vars
			$.each( _this.manifestData, function( pluginId, pluginObj ){
				// Check for flashvars style:
				if( typeof pluginObj == 'string' ){
					uiVarObj[ pluginId ] = pluginObj
				} else {
					for(var key in pluginObj.attributes){
						if( _this.getAttrValue( key ) != null ){
							uiVarObj[ pluginId + '.' + key ] = _this.getAttrValue( key );
						}
					}
				}
			});
		...
		/** 
		 * Gets the edit interface per plugin, 
		 * should be broken into a few sub plugins
		 */
		getAttrEdit: function(){
			var _this = this;
			var $mainPlugin = '';
			var $tbody = $('<tbody />');
			// for each setting get config
			if( _this.manifestData[_this.pluginName] ){
				$.each( _this.manifestData[_this.pluginName].attributes, function( attrName, attr){
			...
			$.each( _this.manifestData, function( otherPluginId, pluginObject ){
				if( pluginObject.attributes && _this.pluginName != otherPluginId  ){
					$otherPlugins.append( 
					...
					$.each( pluginObject.attributes, function( attrName, attr ){
						...
						// setup local pointer to $editVal:
						if( !attr.hideEdit && _this.flashvars[ otherPluginId ][ attrName ] ){
							attr.$editVal = $('<div />').getEditValue( attrName ) ;
			...
			var $fvBody = '';
			var $fvTbody = $('<tbody />');
			$.each( _this.manifestData, function( attrName, attr){
				// check if we should skip the plugin
				if( attrName == _this.pluginName || attr.attributes || attr.hideEdit ){
					return true;
				}
				if( $fvBody == '' ){
					$fvBody = $('<div />').append(
			...
			var $updatePlayerBtn = _this.flashvarCallback ? 
					...
					.click( function(){
						...
						win.location.hash = 'config=' + JSON.stringify(
							_this.getChangedSettingsHash()
						);
						
						_this.flashvarCallback( getConfiguredFlashvars() );
			...
			if( $.browser.msie && $.browser.version < 10 ){
				...
				kWidget.auth.addAuthCallback(function( userObject ){
					var uiConfId = localStorage[ 'kdoc-embed-uiconf_id' ] || _this.pageEmbed.uiconf_id;
					// Create new player: 
					...
					.click( function(){
						...
						api.doRequest( {
							...
							if( isDataError( data )  ){
								$createPlayerBtn.after( _this.getDataError( data ) );
								return ;
							}
				...
				kWidget.auth.addAuthCallback(function( userObject ){
					...
					.click( function(){
						...
						api.doRequest( {
							...
							if( isDataError( data )  ){
								$saveToUiConf.after( _this.getDataError( data ) );
								return ;
							}
							var updatedUiConfString = _this.addUiVarsToUiConf( data.confFile );
							if( updatedUiConfString === false ){
								$saveToUiConf.after( _this.getDataError( {'message': "Could not find uiVars"} ) );
								return; 
							}
		...
		getObjectDiff: function( obj1, obj2 ){
			...
			$.each( obj1, function( pKey, pVal ){
				pVal = _this.noramlizeValue( pVal );
				if( !obj2[ pKey ] || pVal != obj2[ pKey ] ){
						...
						typeof obj2[ pKey ] == 'object' )
					{
						$.each( obj1[ pKey ], function( spKey, spVal ){
							spVal = _this.noramlizeValue( spVal );
		...
		getChangedSettingsHash: function(){
			var _this = this;
			// Get all the edit values that changed ( single config depth )
			var flashVarsChanged = {};
			if( _this.pageEmbed && _this.pageEmbed.flashvars ){
				flashVarsChanged = _this.getObjectDiff( getConfiguredFlashvars(), _this.pageEmbed.flashvars );
			}
			...
			var settingsChanged = _this.getObjectDiff( kWidget.getLocalFeatureConfig( _this.pageEmbed ), _this.pageEmbed ); 
		...
		getEmbed: function(){
			var _this = this;
			var kdp = $('#' + _this.pageEmbed.targetId )[0];
		 ...
		 * Flashvar config tab:
		 */
		getFlashvarConfig: function( baseTabs ){
			var _this = this;
			if( ! baseTabs ){
				baseTabs ='';
			}
			...
			$.each( _this.manifestData, function( pName, attr ){
				...
				if( _this.manifestData[ pName ].attributes){
					fvText+= baseTabs + "\t\"" + pName +'": {' + "\n";
					var aCount =0;
					$.each( _this.manifestData[ pName ].attributes, function( attrName, attr ){
						if( _this.getAttrValue( attrName) !== null ){
							aCount++;
						}
					});
					var aInx =0;
					$.each( _this.manifestData[ pName ].attributes, function( attrName, attr ){
						if( _this.getAttrValue( attrName) !== null ){
							...
							fvText += baseTabs + "\t\t\"" + attrName + '\" : ' + _this.getJsonQuoteValue( attrName ) + aComa +"\n";
						}
					})
					fvText+= baseTabs + "\t}" + coma + "\n";
				} else {
					fvText += baseTabs +  "\t\"" + pName + "\" : " + _this.getJsonQuoteValue( pName ) + coma +"\n";
				}
			});
			fvText+= baseTabs +  "}\n";
			return fvText;
		...
		getFlashvarConfigHTML: function(){
			return $('<div />').append( 
		 ...
		 * UiConf tab
		 */
		getUiConfConfig: function(){
			var _this = this;
			var uiText = '';
			// add uiConf vars
			$.each( _this.manifestData, function( pAttrName, attr ){
				if( _this.manifestData[ pAttrName ].attributes ){
					uiText += '<Plugin id="' + pAttrName + '" ';
					$.each( _this.manifestData[ pAttrName ].attributes, function( attrName, attr){
						if( attrName != 'plugin' && _this.getAttrValue( attrName) !== null ){
							uiText+= "\n\t" + attrName + '="' +  _this.getAttrValue( attrName )  + '" ';
						}
					});
					uiText +="\n/>\n";
					return true;
				}
				uiText += "\n" + '<var key="' + pAttrName + '" value="' + _this.getAttrValue( pAttrName ) +'" />';
			});
		...
		getPlayerStudioLine: function(){
			...
			$.each( _this.manifestData, function( pAttrName, attr ){
				if( _this.manifestData[ pAttrName ].attributes ){
					$.each( _this.manifestData[ pAttrName ].attributes, function( attrName, attr){
						if( _this.getAttrValue( attrName ) != null ){
							plText += and + pAttrName + '.' + attrName + '=' + _this.getAttrValue( attrName );
							and ='&';
						}
					})
					return true;
				...
				// else flat attribute:
				plText += and + pAttrName + '=' + _this.getAttrValue( pAttrName );
				and ='&';
			});
	...
	/** 
	 * Master prettyKalturaConfig jQuery init
	 */
	...
	$.fn.prettyKalturaConfig = function( pluginName, flashvars, flashvarCallback, showSettingsTab, pageEmbed ){
		return this.each(function(){
			new kPrettyConfig({
				...
				'flashvars':flashvars, 
				'flashvarCallback': flashvarCallback, 
	...
	/**
	 * jQuery plugin for pagged input
	 */
	...
	$.fn.getEditValue = function( attrName ){
		...
		switch( _this.getAttrType( attrName ) ){
						...
						$('<ul class="dropdown-menu" />').append(
							$('<li />').append(
								$('<a href="#">true</a>').click(function(){
									...
									_this.setAttrValue(attrName, 'true' );
								})
							),
							$('<li />').append(
								$('<a href="#">false</a>').click(function(){
									// activate button
									$('#btn-update-player-' + id ).removeClass('disabled');
									_this.setAttrValue(attrName, 'false' );
								})
							)
						)
					)
			...
			case 'list':
				...
				var valueObj = _this.getVarObj( attrName );
				var list = valueObj[ 'list' ];
				var valueSet = _this.getAttrValue( attrName ).split(',');
				$.each( list, function( key, title){
					...
					$li = $( '<li> ')
						.append( 
							...
							.text( title )
							.click(function( event ){
								valueSet = _this.getAttrValue( attrName ).split(',');
								valueIndex = $.inArray( key, valueSet );
								if( valueIndex == -1 ){
									$( this ).prepend( $('<i class="icon-ok">' ) );	
									_this.setAttrValue( attrName, valueSet.join( ',' ) + ',' + key );
								} else {
									$( this ).find('i').remove();
									valueSet.splice( valueIndex, 1 );
									_this.setAttrValue( attrName, valueSet.join( ',' ) );
								}
				...
				$.each( enumList, function( inx, eVal ){
					$enumUlList.append(
						...
						.click(function(){
							// activate button
							$('#btn-update-player-' + id ).removeClass('disabled');
							_this.setAttrValue( attrName, eVal );
						})	
					)
				});
				$( this ).html(
			...
			case 'color':
				var getHtmlColor = function(){
					return ( _this.getAttrValue( attrName ) + "" ).replace('0x', '#' );
				}
				...
				$colorSelector.ColorPicker({
					...
					onChange: function (hsb, hex, rgb) {
						...
						// activate button
						$('#btn-update-player-' + id ).removeClass('disabled');
						_this.setAttrValue( attrName, '0x' + hex );
					}
				});
			break;
			case 'string':
			default:
				...
				var getValueDispaly = function( attrName ){
					var attrValue = _this.getAttrValue( attrName ) || '<i>null</i>';
					if( _this.getAttrType( attrName ) == 'url'  &&  _this.getAttrValue( attrName ) !== null ){
						attrValue = $('<span />').append(
							$('<a />').attr({
								'href': _this.getAttrValue( attrName ),
								'target' : "_new"
							}).append(
				...
				$( this ).css('overflow-x', 'hidden').html( getValueDispaly( attrName ) ).click(function(){
					...
					activeEdit = true;
					$( this ).html( 
						$('<input type="text" style="width:100px" />').val( _this.getAttrValue( attrName ) )
					);
					...
					.blur( function() {
						// activate button
						$('#btn-update-player-' + id ).removeClass('disabled');
						_this.setAttrValue( attrName, $(this).val() );
