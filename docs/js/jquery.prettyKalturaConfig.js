...
(function( $ ){
	...
	return this.each(function() {
	    ...
	    */
		...
		switch( getAttrType( attrName ) ){
		    ...
		    case 'list':
			$( this ).empty();
			var $listBtnGroup = $('<div class="btn-group">').append(
			    '<a class="btn dropdown-toggle" data-toggle="dropdown" href="#">' +
			    'Set active' +
			    '<span class="caret"></span>' +
			    '</a>'
			    );
			var $listUl =$( '<ul class="dropdown-menu">');
	    ...
	    */
		...
		api.doRequest( {
			...
			$saveToUiConf.after( getDataError( {
			    'message': "Could not find uiVars"
			} ) );
			return; 
		    }
		...
		api.doRequest( {
			...
			if ( resultName === null) {
			    ...
			    var updatedUiConfString = addUiVarsToUiConf( data.confFile );
			    if( updatedUiConfString === false ){
				$createPlayerBtn.after( getDataError( {
				    'message': "Could not find uiVars"
				} ) );
				return; 
			    }
	...
	var doPagedTable = function( pageInx, sortKey ){
		...
		if( data.totalCount > pageSize  ){
		    ...
		    var $pager = $('<ul>').appendTo( $pagerContainer );
		    var $pLink = $('<li><a href="#">&#x00ab;</a></li>').click( function(){
			return doPaged( pageInx-1 )
		    } )
		    if( pageInx == 0 ){
			...
			(function(inx){
			    var $numLink = $('<li><a href="#">' + (inx ) + '</a></li>').click( function(){
				return doPaged( inx )
			    } )
			    if( pageInx == inx  ){
		    ...
		    var $nLink = $('<li><a href="#">&#x00bb;</a></li>').click( function(){
			return doPaged( pageInx+1 )
		    } )
	...
	.click( function( event ){
		...
		$uiConfList.fadeOut('fast', function(){
		    $(this).remove()
		}) 
	    });
