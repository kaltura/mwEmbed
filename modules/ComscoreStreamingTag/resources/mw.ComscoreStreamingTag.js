...
mw.ComscoreStreamingTag.prototype = {
	...
	setupConfig: function() {
		...
		this.config = this.embedPlayer.getKalturaConfig( 'comscorestreamingtag', attributes );
	},
	...
	loadXML: function( callback ) {
		...
		if( this.config.cTagsMap ){
			mw.log('ComscoreStreamingTag:: Retrive ComscoreStreamingTag xml file from: ' + _this.config.cTagsMap);
			new mw.ajaxProxy({
				...
				error: function() {
					mw.log("ComscoreStreamingTag:: Error: failed to load: " + _this.config.cTagsMap);
	...
	handleXMLResult: function( xml, callback ) {
		var _this = this;
		mw.log('ComscoreStreamingTag:: loaded xml successfully, setup cparams & player bindings');
	...
	addPlayerBindings: function( callback ) {
		...
		embedPlayer.bindHelper('onplay' + this.bindPostfix, function() {
			if ( !playerPlayedFired && !_this.inAd() ){
				...
				mw.log('ComscoreStreamingTag:: Send Content Start Beacon');
				_this.comScoreStreamingTagBeacon( cParams );
			}
		});
		...
		embedPlayer.bindHelper('AdSupport_StartAdPlayback' + this.bindPostfix, function( event, adType ) {
			...
			// Send beacon
			mw.log('ComscoreStreamingTag:: Send Ad Start Beacon');
			_this.comScoreStreamingTagBeacon( cParams );
			shouldSendBeacon = false;
		...
		embedPlayer.bindHelper('AdSupport_EndAdPlayback' + this.bindPostfix, function() {
			if( sendOnSequnceEnd ) {
				...
				mw.log('ComscoreStreamingTag:: Send Ad End Beacon (Resume Content)');
				_this.comScoreStreamingTagBeacon( cParams );
				sendOnSequnceEnd = false;
			}
		});
		...
		embedPlayer.bindHelper('monitorEvent' + this.bindPostfix, function() {
			if( shouldSendBeacon ) {
				cParams["c5"] = _this.parseCAttribute('c5'); // Reset C5
				mw.log('ComscoreStreamingTag:: Send Resume Content Beacon (No Ad)');
				_this.comScoreStreamingTagBeacon( cParams );
				shouldSendBeacon = false;
			}
		});
	...
	setupCParams: function() {
		mw.log('ComscoreStreamingTag:: Setup CParams');
		...
		/**
		 * For debug:
		 console.log( 'ComscoreStreamingTag config: ', this.config);
	 ...
	 *
	 * <Plugin id="comscorestreamingtag" ... c3="defaultValue" c3attributeKey="name" c3attributeValue="id" c3Value="{Metadata.content.contentOwnerName}" ... />
	...
	comScoreStreamingTagBeacon: function( beaconObject ) {
		var _this = this;
		...
		mw.log('ComscoreStreamingTag:: Sent Beacon: ' + loadUrl, beaconObject);
	},
	destroy: function() {
