/**
 * The RaptMediaNgScrubber adds scrubber override capabilities to support RaptMedia clip context.
...
(function ( mw, $ ) {
	"use strict";
	mw.PluginManager.add('raptMediaNgScrubber', mw.PluginManager.getClass('scrubber').extend({
		getSliderConfig: function() {
			...
			config.change = function(event, ui) {
				var status = embedPlayer.evaluate('{raptMediaNg.status}');
				var segment = embedPlayer.evaluate('{raptMediaNg.currentSegment}');
				...
				if (embedPlayer.userSlide) {
					...
					embedPlayer.triggerHelper("raptMediaNg_doSeek", seekTime);
				}
			};
		...
		getDuration: function(ui) {
			...
			if (player.evaluate('{raptMediaNg.status}') === 'enabled') {
				return player.evaluate('{raptMediaNg.currentSegment.duration}');
			} else {
