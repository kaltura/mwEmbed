...
mw.Widevine = function (embedPlayer, callback) {
	return this.init( embedPlayer, callback );
}

mw.Widevine.prototype = {	
	player: null,
	init: function(  embedPlayer, callback){
		mw.Widevine.player = embedPlayer;
		kWidget.addReadyCallback( function( playerId ){

			//if Kplayer is available - tell it to load widevine swf plugin
			if ( kWidget.supportsFlash() ) {
				embedPlayer.setKalturaConfig('kdpVars', 'widevine', { plugin: 'true', loadingPolicy: 'preInitialize', asyncInit: 'true'});
			//	embedPlayer.setKalturaConfig('kdpVars.widevine', 'loadingPolicy', 'preInitialize');
			//	embedPlayer.setKalturaConfig('kdpVars.widevine', 'asyncInit', 'true');
				
				embedPlayer.addJsListener( 'entryReady', function () {
					if ( widevine.isWvFlavors() ) {
						embedPlayer.setKalturaConfig('kdpVars', 'widevine', {isWv: true});
					}
			       	widevine.init();
			    });
			}
				...
				//hide default "no source found" alert
			     embedPlayer.setKalturaConfig(null, 'disableAlerts', true);
			     
			     embedPlayer.addJsListener("playerReady", function () {
				    var flavors = embedPlayer.mediaElement.getPlayableSources();
				    ...
				    if (kWidget.isMobileDevice()) {
						 msg = mw.getConfig( 'Widevine.useSupportedDeviceMsg') || "This video requires Adobe Flash Player, which is not supported by your device. You can watch it on devices that support Flash." ;
						 title = mw.getConfig( 'Widevine.useSupportedDeviceTitle') || "Notification";
				    } else {
					 	//flash is not installed - prompt to install flash
						if (navigator.mimeTypes ["application/x-shockwave-flash"] == undefined) {
						     msg = mw.getConfig( 'Widevine.intallFlashMsg') || "This video requires Adobe Flash Player, which is currently not available on your browser. Please <a href='http://www.adobe.com/support/flashplayer/downloads.html' target='_blank'> install Adobe Flash Player </a> to view this video.";
						     title = mw.getConfig( 'Widevine.installFlashTitle') || "Notification";
						} else { //else prompt to use kdp
						     msg = mw.getConfig( 'Widevine.useKdpMsg') || "This video requires Adobe Flash enabled player.";
						     title = mw.getConfig( 'Widevine.useKdpTitle') || "Notification";
						}
				    }
				    if (msg && title) {
				    	embedPlayer.layoutBuilder.displayAlert( {keepOverlay:true, message: msg , title: title} );
...
var widevine = function() {
	...
	function showDownloadPageText(){
		...
		//mw.Widevine.embedPlayer..trigger( 'seeking' );("noWidevineBrowserPlugin");
		
		if (window.wvPromptDiv)
			return window.wvPromptDiv;
			
		//get texts and style from the player, if they were set
		var wvPromptStyle = mw.getConfig( 'Widevine.promptStyle') 
							|| 'border:solid 1px #eeeeee; position:fixed; z-index:" + zIndex + "; width:100%; height:40px; color:#505050; background-color:#FDFFDB; top:0px; right:0px; left:0px; font-family:arial; font-size:12px;';
		var wvPromptText = mw.getConfig( 'Widevine.promptText')
							|| "Widevine Video Optimizer plugin is needed for enabling video playback in this page. ";
		var wvPromptLinkText = mw.getConfig( 'Widevine.promptLinkText')
							|| 'Get Video Optimizer';
        var wvPromptInfoText = mw.getConfig( 'Widevine.promptInfoText');
        var wvPromptInfoLink = mw.getConfig( 'Widevine.promptInfoLink');
        var wvPromptRestartChromeAfterInstall = mw.getConfig( 'Widevine.PromptRestartChromeAfterInstall') 
	...
	isWvFlavors: function() {
		var entryFlavors = mw.Widevine.player.mediaElement.getPlayableSources();
