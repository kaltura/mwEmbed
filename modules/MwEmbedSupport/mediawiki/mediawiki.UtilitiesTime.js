...
( function ( mw ) {
	 ...
	 * @param {Boolean}
	 *			mmFormat if you want to show 2 digits for minutes
	 * @return {Float} String npt format
	 */
	mw.seconds2npt = function ( sec, showMs, mmFormat ) {
		if ( isNaN( sec ) ) {
	...
	*/
	...
	mw.npt2seconds = function ( nptStr ) {
		...
		return parseInt( hour * 3600, 10 ) + parseInt( min * 60, 10 ) + parseFloat( sec, 10 );
	};
