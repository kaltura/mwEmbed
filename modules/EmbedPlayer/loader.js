...
(function(mw) {
	 ...
	 * 
	 * We use mw.addSetupHook instead of mw.ready so that mwEmbed player is
	 * setup before any other mw.ready calls
	 */
			...
			$j(mw.getConfig('EmbedPlayer.RewriteTags')).each(
					function(index, element) {
						...
						if ($j(element).attr("id") == '') {
							$j(element).attr("id",
									'v' + (rewriteElementCount++));
						}
						// Add an absolute positioned loader
						$j(element).getAbsoluteOverlaySpinner().attr('id',
								'loadingSpinner_' + $j(element).attr('id'))
	...
	mw.addModuleLoader('EmbedPlayer', function() {
		...
		// Hide videonojs class
			...
			// Set up the embed video player class request: (include the skin js
			// as well)
			var dependencyRequest = [
					[ 'mw.EmbedPlayer' ],
					[ 'mw.PlayerControlBuilder', '$j.fn.hoverIntent',
							'mw.style.EmbedPlayer', '$j.cookie',
							// Add JSON lib if browsers does not define "JSON"
							// natively
							'JSON', '$j.ui', '$j.widget' ],
					[ '$j.ui.mouse', '$j.fn.menu', 'mw.style.jquerymenu',
							'$j.ui.slider' ] ];
			...
			// Do short detection, to avoid extra player library request in
			// ~most~ cases.
			// ( If browser is firefox include native, if browser is IE include
			// java )
			if ($j.browser.msie) {
	 ...
	 * 
	 * For example a player skin class name could result in adding some css and
	 * javascirpt to the player library request.
	 * 
	 * @param {Object}
	 *			playerElement The tag to check for library dependent request
	 *            classes.
	 * @param {Array}
	 *			dependencyRequest The library request array
	 */
	mw.embedPlayerUpdateLibraryRequest = function(playerElement,
		...
		if (!skinName
				|| $j.inArray(skinName.toLowerCase(), mw.validSkins) == -1) {
