...
(function(mw, $) {
	 ...
	 * 
	 * @param the
	 *            embedPlayer element we are targeting
	 */
	...
	mw.PlayerControlBuilder.prototype = {
		...
		// Default supported components is merged with embedPlayer set of
		// supported types
		supportedComponents : {
		 ...
		 * 
		 * @param {Object}
		 *            embedPlayer EmbedPlayer interface
		 */
		init : function(embedPlayer) {
			...
			if (mw['PlayerSkin' + skinClass]) {
				...
				var _this = $.extend(true, {}, this, mw['PlayerSkin'
						+ skinClass]);
				return _this;
			}
		...
		addControls : function() {
			...
			if (_this.isOverlayControls()) {
				...
				embedPlayer.height = embedPlayer.$interface.height()
						- this.getHeight();
		...
		addControlComponents : function() {
			...
			mw.log('PlayerControlsBuilder:: addControlComponents into:'
					+ this.available_width);
			// Build the supportedComponents list
			this.supportedComponents = $.extend(this.supportedComponents,
			...
			// Check if we have multiple playable sources ( if only one source
			// don't display source switch )
			...
			var addComponent = function(component_id) {
				if (_this.supportedComponents[component_id]) {
					if (_this.available_width > _this.components[component_id].w) {
						...
						mw
								.log('PlayerControlBuilder:: Not enough space for control component:'
										+ component_id);
					}
				}
			};
			...
			for ( var component_id in this.components) {
				...
				// Special case with playhead and time ( to make sure they are
				// to the left of everything else )
				if (component_id == 'playHead' || component_id == 'timeDisplay') {
					continue;
				}

				// Skip "fullscreen" button for assets or where height is 0px (
				// audio )
		 ...
		 * 
		 * @@TODO This has similar logic to mw.embedPlayerNative
		 *        applyIntrinsicAspect we should look at merging their
		 *        functionality.
		 * 
		 * @param {object}
		 *            windowSize object that set { 'width': {width},
		 *            'height':{height} } of target window
		...
		getAspectPlayerWindowCss : function(windowSize) {
			...
			if (targetHeight > windowSize.height) {
				...
				targetWidth = parseInt(targetHeight
						* _this.getIntrinsicAspect());
			}
			var offsetTop = 0;
			// Move the video down 1/2 of the difference of window height
			offsetTop += (targetHeight < windowSize.height) ? (windowSize.height - targetHeight) / 2
					: 0;
			// if the video is very tall in a short window adjust the size:
			var offsetLeft = (targetWidth < windowSize.width) ? parseInt(windowSize.width
					- targetWidth) / 2
					: 0;

			mw.log('PlayerControlBuilder::getAspectPlayerWindowCss: ' + ' h:'
					+ targetHeight + ' w:' + targetWidth + ' t:' + offsetTop
					+ ' l:' + offsetLeft);
			return {
				'position' : 'absolute',
		 ...
		 * 
		 * @return {float} size object with width and height
		 */
		getIntrinsicAspect : function() {
			...
			if (this.embedPlayer.mediaElement
				...
				// see if we have a hardcoded aspect to the source ( Adaptive
				// streams don't have width / height )
				if (ss.aspect) {
					return ss.aspect;
				}
			...
			// check for posterImage size: ( should have Intrinsic aspect size
			// as well )
		...
		getPlayButtonPosition : function(size) {
			...
			var topCompoentOffset = (this.isOverlayControls()) ? pheight
					: pheight / 2;
			return {
				'position' : 'absolute',
				'left' : ((parseInt(size.width) - this
						.getComponentWidth('playButtonLarge')) / 2),
				'top' : ((parseInt(size.height) - topCompoentOffset) / 2)
			};
		},

		/**
		 * Toggles full screen by calling doFullScreenPlayer to enable
		 * fullscreen mode restoreWindowPlayer to restore window mode
		 */
		...
		doFullScreenPlayer : function(callback) {
			...
			// Check for native support for fullscreen and we are in an iframe
			// server
			if (window.fullScreenApi.supportsFullScreen
					&& mw.getConfig('EmbedPlayer.IsIframeServer')) {
				var parentWindow = window.parent;
				var parentTarget = parentWindow.document
						.getElementById(this.embedPlayer.id);
				// Add a binding to catch "escape" fullscreen
				parentTarget.addEventListener(
						fullScreenApi.fullScreenEventName, function(event) {
				...
				// There is a bug with mozfullscreenchange event in all versions
				// of firefox with supportsFullScreen
				// https://bugzilla.mozilla.org/show_bug.cgi?id=724816
				// so we have to have an extra binding to check for size change
				// and then restore.
				if ($.browser.mozilla) {
					// put in a timeout to give the browser time to ~enter~
					// fullscreen initially.
					setTimeout(function() {
						$(window).bind('resize.postFullScreenResize',
								function() {
			...
			// Add a secondary fallback resize ( sometimes iOS loses the $(
		...
		syncPlayerSize : function() {
			mw.log("PlayerControlBuilder::syncPlayerSize: window:"
					+ $(window).width() + ' player: ' + $(embedPlayer).width());
			var embedPlayer = this.embedPlayer;
			// resize to the playlist container
			// TODO change this to an event so player with interface around it (
			// ppt widget etc ) can
		...
		doFullScreenPlayerDom : function() {
			...
			// Add the css fixed fullscreen black overlay as a sibling to the
			// video element
			$interface.after($('<div />').addClass('mw-fullscreen-overlay')
			...
			if (embedPlayer.isPersistentNativePlayer()) {
				$(embedPlayer.getPlayerElement())
						.css(
								{
									'z-index' : mw
											.getConfig('EmbedPlayer.FullScreenZIndex') + 1,
									'position' : 'absolute'
								});
			}
			...
			// Resize the player keeping aspect and with the widow scroll
			// offset:
			...
			$interface
					...
					.each(
							function() {
								// mw.log(' parent : ' + $( this ).attr('id' ) +
								// ' class: ' + $( this ).attr('class') + ' pos:
								// ' + $( this ).css( 'position' ) );
								if ($(this).css('position') == 'absolute') {
									_this.parentsAbsolute.push($(this));
									$(this).css('position', null);
									mw
											.log('PlayerControlBuilder::  should update position: '
													+ $(this).css('position'));
								}
							});
				...
				var checkMovedMouse = function() {
					if (_this.inFullScreen) {
						if (_this.mouseMovedFlag) {
							...
							// Once we move the mouse keep displayed for 4
							// seconds
		...
		// Display a fullscreen tip if configured to do and the browser supports
		// it.
		displayFullscreenTip : function() {
			...
			var toolTipMsg = (navigator.userAgent.indexOf('Mac OS X') != -1) ? gM('mwe-embedplayer-fullscreen-tip-osx')
					: gM('mwe-embedplayer-fullscreen-tip');

			var $targetTip = this.addWarningBinding(
					'EmbedPlayer.FullscreenTip', $('<h3/>').html(toolTipMsg));
		...
		restoreWindowPlayer : function() {
			...
			// Check for native support for fullscreen and support native
			// fullscreen restore
			if (window.fullScreenApi.supportsFullScreen
					&& mw.getConfig('EmbedPlayer.IsIframeServer')) {
				var parentWindow = window.parent;
			...
			// Check if iFrame mode ( fullscreen is handled by the iframe parent
			// dom )
			if (!mw.getConfig('EmbedPlayer.IsIframeServer')) {
				this.restoreWindowPlayerDom();
			} else {
				// if an iframe server make sure the player size is in sync with
				// the iframe window size:
		...
		restoreWindowPlayerDom : function() {
			...
			var $interface = embedPlayer.$interface;
			var interfaceHeight = (_this.isOverlayControls()) ? embedPlayer
					.getHeight() : embedPlayer.getHeight() + _this.getHeight();

			// only animate if we are not inside an iframe
			var aninmate = !mw.getConfig('EmbedPlayer.IsIframeServer');

			mw.log('restoreWindowPlayer:: h:' + interfaceHeight + ' w:'
					+ embedPlayer.getWidth());
			$('.mw-fullscreen-overlay').fadeOut('slow');

			mw.log('restore embedPlayer:: ' + embedPlayer.getWidth() + ' h: '
			...
			embedPlayer.resizePlayer({
				...
				'height' : embedPlayer.getHeight()
					...
					function() {
						...
						$(
								[ $interface, $interface.find('.playerPoster'),
										embedPlayer ]).css(topPos);
		...
		resizePlayer : function(size, animate, callback) {
			mw.log("PlayerControlBuilder:: resizePlayer: " + size.width + ' '
					+ size.height);
			var _this = this;
			...
			if (!_this.isOverlayControls()) {
				buttonScale['height'] = buttonScale['height']
						- this.getHeight();
			}
			...
			if (animate) {
				...
				$interface.find('.play-btn-large').animate(
						_this.getPlayButtonPosition(buttonScale));
		...
		getOverlayWidth : function() {
			return (this.embedPlayer.getPlayerWidth() < 300) ? 300
		...
		getOverlayHeight : function() {
			return (this.embedPlayer.getPlayerHeight() < 200) ? 200
					: this.embedPlayer.getPlayerHeight();
		},

		/**
		 * addControlBindings Adds control hooks once controls are in the DOM
		 */
		addControlBindings : function() {
			...
			$(embedPlayer).bind('onplay' + this.bindPostfix, function() { // Only
																			// bind
																			// once
																			// played
			...
			$(embedPlayer).bind(
					'onEnableInterfaceComponents' + this.bindPostfix,
					function() {
			...
			$(embedPlayer).bind(
					'onDisableInterfaceComponents' + this.bindPostfix,
					function() {
			...
			if (!_this.isOverlayControls()) {
				...
				$(embedPlayer).bind('touchstart' + this.bindPostfix,
						function() {
							if (embedPlayer.paused) {
								embedPlayer.play();
							} else {
				...
				$(embedPlayer).bind(
						'touchstart' + this.bindPostfix,
						function() {
							if (embedPlayer.$interface.find('.control-bar').is(
									':visible')) {
								if (embedPlayer.paused) {
									embedPlayer.play();
								} else {
							...
							_this.hideControlBarCallback = setTimeout(
									function() {
										_this.hideControlBar()
									}, 5000);
				...
				var hoverIntentConfig = {
					...
					'over' : function(e) {
						...
						// Show controls with a set timeout ( avoid fade in fade
						// out on short mouse over )
						_this.showControlBar();
						bindSpaceUp();
					},
					'out' : function(e) {
				...
				// special check for IE9 ( does not count hover on non-visiable
				// inerface div
				if (mw.isIE9()) {
					$(embedPlayer.getPlayerElement()).hoverIntent(
							hoverIntentConfig);

					// Add hover binding to control bar
					embedPlayer.$interface.find('.control-bar').hover(
							function(e) {
								_this.onControlBar = true;
								embedPlayer.$interface.find('.control-bar')
										.show();
							},
							function(e) {
			...
			if (this.addSkinControlBindings
		...
		removePlayerClickBindings : function() {
			$(this.embedPlayer).unbind("click" + this.bindPostfix).unbind(
					"dblclick" + this.bindPostfix);
		},
		addPlayerClickBindings : function() {
			...
			// Setup "dobuleclick" fullscreen binding to embedPlayer ( if
			// enabled )
			...
			$(embedPlayer).bind(
					...
					function() {
						...
						setTimeout(function() {
							// check if no click has since the time we called
							// the setTimeout
							if (!didDblClick) {
								if (embedPlayer.paused) {
									embedPlayer.play();
		...
		hideControlBar : function() {
			...
			// Hide the control bar
			this.embedPlayer.$interface.find('.control-bar').fadeOut(
					animateDuration);
		...
		showControlBar : function(keepOnScreen) {
			...
			mw.log('PlayerControlBuilder:: ShowControlBar,  keep on screen: '
					+ keepOnScreen);

			// Show interface controls
			this.embedPlayer.$interface.find('.control-bar').fadeIn(
					animateDuration);
		...
		checkNativeWarning : function() {
			...
			var supportingPlayers = mw.EmbedTypes.getMediaPlayers()
			...
			// Check for h264 and or flash/flv source and playback support and
			// don't show warning
			if ((mw.EmbedTypes.getMediaPlayers().getMIMETypePlayers(
					'video/h264').length && this.embedPlayer.mediaElement
					.getSources('video/h264').length)
					|| (mw.EmbedTypes.getMediaPlayers().getMIMETypePlayers(
							'video/x-flv').length && this.embedPlayer.mediaElement
							.getSources('video/x-flv').length)
					|| (mw.EmbedTypes.getMediaPlayers().getMIMETypePlayers(
							'application/vnd.apple.mpegurl').length && this.embedPlayer.mediaElement
							.getSources('application/vnd.apple.mpegurl').length)
					|| (mw.EmbedTypes.getMediaPlayers().getMIMETypePlayers(
							'audio/mpeg').length && this.embedPlayer.mediaElement
							.getSources('audio/mpeg').length)) {
		 ...
		 * 
		 * @param {string}
		 *            preferenceId The preference Id
		 * @param {object}
		 *            warningMsg The jQuery object warning message to be
		 *            displayed.
		 * 
		 */
		/**
		 * Display a warning message on the player checks a preference Id to
		...
		addWarningBinding : function(preferenceId, warningMsg) {
			mw.log('mw.PlayerControlBuilder: addWarningBinding: '
			...
			$targetWarning.append($('<label />').text(
					gM('mwe-embedplayer-do_not_warn_again')).attr('for',
					...
					.hoverIntent(
							{
								...
								'over' : function() {
									...
									// Check the global config before showing
									// the warning
									if (mw.getConfig(preferenceId) === true
											&& $.cookie(preferenceId) != 'hidewarning') {
										mw
												.log("WarningBindinng:: show warning "
														+ mw
																.getConfig(preferenceId)
														+ ' cookie: '
														+ $
																.cookie(preferenceId)
														+ 'typeof:'
		...
		doVolumeBinding : function() {
			var embedPlayer = this.embedPlayer;
			var _this = this;
			embedPlayer.$interface.find('.volume_control').unbind()
			...
			if (this.volume_layout == 'vertical') {
				...
				embedPlayer.$interface.find('.volume_control').hover(
						function() {
							...
							if (embedPlayer && embedPlayer.isPlaying
									&& embedPlayer.isPlaying()
									&& !embedPlayer.supports['overlays']) {
								$targetvol.removeClass('vol_container_top')
			...
			var sliderConf = {
				...
				slide : function(event, ui) {
					...
					mw.log('PlayerControlBuilder::slide:update volume:'
							+ percent);
				...
				change : function(event, ui) {
					var percent = ui.value / 100;
					if (percent == 0) {
						embedPlayer.$interface.find('.volume_control span')
								.removeClass('ui-icon-volume-on').addClass(
										'ui-icon-volume-off');
					} else {
						embedPlayer.$interface.find('.volume_control span')
								.removeClass('ui-icon-volume-off').addClass(
										'ui-icon-volume-on');
					}
					mw.log('PlayerControlBuilder::change:update volume:'
							+ percent);
		...
		getOptionsMenu : function() {
			...
			for ( var menuItemKey in this.optionMenuItems) {
				...
				if ($.inArray(menuItemKey, mw
						.getConfig('EmbedPlayer.EnabledOptionsMenuItems')) === -1) {
					continue;
				}
		...
		/**
		 * Updates the player status that displays short text msgs and the play
		 * clock
		 * 
		 * @param {String}
		 *            value Status string value to update
		 */
		 ...
		 * Option menu items
		 * 
		 * @return 'li' a li line item with click action for that menu item
		 */
		optionMenuItems : {

			/*
			 * Commented out, replaced by flavor selector TODO: need to remove
			 * all switch player related code // Player select menu item
			 * 'playerSelect': function( ctrlObj ){ if( mw.isIpad() ){ return
			 * []; } return $.getLineItem( gM( 'mwe-embedplayer-choose_player' ),
			 * 'gear', function( ) { ctrlObj.displayMenuOverlay(
			 * ctrlObj.getPlayerSelect() ); } ); },
			 */
			/*
			 * Commented out, replaced by 'download button' configured by uiConf
			 * TODO: need to remove all related code // Download the file menu
			 * 'download': function( ctrlObj ) { if( mw.isIpad() ) return false;
			 * return $.getLineItem( gM( 'mwe-embedplayer-download' ), 'disk',
			 * function( ) { ctrlObj.displayMenuOverlay( gM('mwe-loading_txt' ) ); //
			 * Call show download with the target to be populated
			 * ctrlObj.showDownload( ctrlObj.embedPlayer.$interface.find(
			 * '.overlay-content' ) ); $( ctrlObj.embedPlayer ).trigger(
			 * 'showDownloadEvent' ); } ); },
			 */
			// Share the video menu
			'share' : function(ctrlObj) {
				return $.getLineItem(gM('mwe-embedplayer-share'),
						'mail-closed', function() {
			...
			'aboutPlayerLibrary' : function(ctrlObj) {
				return $.getLineItem(gM('mwe-embedplayer-about-library'),
						'info', function() {
							ctrlObj.displayMenuOverlay(ctrlObj
									.aboutPlayerLibrary());
							$(ctrlObj.embedPlayer)
		...
		closeMenuOverlay : function() {
			var _this = this;
			var embedPlayer = this.embedPlayer;
			var $overlay = embedPlayer.$interface
					.find('.overlay-win,.ui-widget-overlay,.ui-widget-shadow');
		 ...
		 * 
		 * @param {String}
		 *            overlayContent content to be displayed
		 */
		displayMenuOverlay : function(overlayContent, closeCallback,
			...
			// Add an overlay
			embedPlayer.$interface.append($('<div />').addClass(
			...
			if (!hideCloseButton) {
				...
				$closeButton = $('<div />').addClass(
						'ui-state-default ui-corner-all ui-icon_link rButton')
						.css({
							'position' : 'absolute',
							'cursor' : 'pointer',
							'top' : '2px',
			...
			var controlBar_height = embedPlayer.$interface.find('.control-bar')
					.height();
			...
			var $overlayMenu = $('<div />')
					.addClass(
							'overlay-win ui-state-default ui-widget-header ui-corner-all')
					.css(overlayMenuCss).append(
							$closeButton,
							$('<div />').addClass('overlay-content').append(
									overlayContent));
			...
			shadowCss['width'] = 260;
			shadowCss['z-index'] = 2;
			var $overlayShadow = $('<div />').addClass(
					'ui-widget-shadow ui-corner-all').css(shadowCss);

			// Append the overlay menu to the player interface
			embedPlayer.$interface.prepend($overlayMenu
			// ,$overlayShadow
			).find('.overlay-win').fadeIn("slow");
		 ...
		 * 
		 * @param (Object)
		 *            Object which includes: title Alert Title body Alert body
		 *            buttonSet[label,callback] Array of buttons style CSS
		 *            object
		 * 
		 */
		displayAlert : function(alertObj) {
			...
			if (typeof alertObj.callbackFunction == 'string') {
				...
				// Make life easier for internal usage of the listener mapping
				// by supporting
			...
			var $container = $('<div />').attr('id', 'alertContainer')
					.addClass('alert-container');
			var $title = $('<div />').text(alertObj.title).addClass(
					'alert-title alert-text');
			if (alertObj.props && alertObj.props.titleTextColor) {
				$title.removeClass('alert-text');
				$title.css('color', mw
						.getHexColor(alertObj.props.titleTextColor));
			}
			var $message = $('<div />').text(alertObj.message).addClass(
			...
			if (alertObj.props && alertObj.props.buttonRowSpacing) {
				$buttonsContainer.css('margin-top',
						alertObj.props.buttonRowSpacing);
			}
			...
			$.each($buttonSet, function(i) {
				...
				var $currentButton = $('<button />').css({
					'padding' : '5px',
					'font-size' : '12px'
				}).text(label).click(function(eventObject) {
				...
				if (buttonsNum > 1) {
					if (i < buttonsNum - 1) {
						if (alertObj.props && alertObj.props.buttonSpacing) {
							$currentButton.css('margin-right',
									alertObj.props.buttonSpacing);
						}
					}
				}
			...
			return embedPlayer.controlBuilder.displayMenuOverlay($container,
					false, true);
		},
											...
											gM(
													...
													$('<a />')
															.attr(
																	{
																		'href' : mw
																				.getConfig('EmbedPlayer.LibraryPage'),
																		'target' : '_new'
																	}))));
		},
		/**
		...
		getShare : function() {
			...
			$shareList.append($('<li />').text(
					gM('mwe-embedplayer-embed_site_or_blog'))
			/*
			 * .append( $('<a />') .attr('href', '#') .addClass( 'active' )
			 * .text( gM( 'mwe-embedplayer-embed_site_or_blog' ) ) )
			 */
			);

			$shareInterface.append($('<h2 />').text(
					gM('mwe-embedplayer-share_this_video')));

			$shareInterface.append($shareList);

			var $shareButton = false;
			if (!mw.isIpad()) {
				$shareButton = $('<button />').addClass(
						'ui-state-default ui-corner-all copycode').text(
						gM('mwe-embedplayer-copy-code')).click(function() {
			...
			$('<textarea />').attr('rows', 4).html(embed_code).click(
					function() {
						$(this).select();
					}),
		 ...
		 * 
		 * @param {Object}
		 *            $target jQuery target for output
		 */
		getPlayerSelect : function() {
			...
			var $playerSelect = $('<div />').append(
					$('<h2 />').text(gM('mwe-embedplayer-choose_player')));
			...
			$
					.each(
							...
							function(sourceId, source) {
								...
								var isPlayable = (typeof mw.EmbedTypes
										.getMediaPlayers().defaultPlayer(
												source.getMIMEType()) == 'object');
								var isSelected = (source.getSrc() == embedPlayer.mediaElement.selectedSource
										.getSrc());

								$playerSelect.append($('<h3 />').text(
										source.getTitle()));
								...
								if (isPlayable) {
									...
									var supportingPlayers = mw.EmbedTypes
											.getMediaPlayers()
											.getMIMETypePlayers(
													source.getMIMEType());
									...
									for ( var i = 0; i < supportingPlayers.length; i++) {
										...
										// already selected )
										if (embedPlayer.selectedPlayer.id == supportingPlayers[i].id
												&& isSelected) {
											...
											var $playerLine = $('<span />')
													...
													.click(
															function() {
																embedPlayer.controlBuilder
																		.closeMenuOverlay();
															});
											// .addClass( 'ui-state-highlight
											// ui-corner-all' ); removed by ran
										} else {
											// Non active player add link to
											// select:
											$playerLine = $('<a />')
													.attr(
															{
													...
													.click(
															function() {
																...
																mw
																		.log('PlayerControlBuilder:: source id: '
																				+ sourceId
																				+ ' player id: '
																				+ player_id);
																...
																embedPlayer.mediaElement
																		.setSourceByIndex(sourceId);
																var playableSources = embedPlayer.mediaElement
																		.getPlayableSources();
										...
										// Add the player line to the player
										// list:
										$playerList.append($('<li />').append(
												$playerLine));
									}
									...
									// No player available:
									$playerSelect.append(gM(
											'mwe-embedplayer-no-player', source
													.getTitle()));
								}
							});
		 ...
		 * 
		 * @param {Object}
		 *            $target jQuery target to output to
		 */
		...
		showDownloadWithSources : function($target) {
			var _this = this;
			mw.log('PlayerControlBuilder:: showDownloadWithSources::'
					+ $target.length);
			var embedPlayer = this.embedPlayer;
			// Empty the target:
			$target.empty();
			...
			$.each(embedPlayer.mediaElement.getSources(), function(index,
				...
				if (source.getSrc()) {
					...
					var $dl_line = $('<li />').append(
							$('<a />').attr('href', source.getSrc()).text(
									source.getTitle()));
			...
			if ($mediaList.find('li').length != 0) {
				$target.append($('<h2 />').text(
						gM('mwe-embedplayer-download_full')), $mediaList);
			}

			if ($textList.find('li').length != 0) {
				$target.append($('<h2 />').html(
						gM('mwe-embedplayer-download_text')), $textList);
			}
		},
		getSwitchSourceMenu : function() {
			...
			function addToSourceMenu(source) {
				...
				var icon = (source.getSrc() == embedPlayer.mediaElement.selectedSource
						.getSrc()) ? 'bullet' : 'radio-on';
				$sourceMenu
						.append($
								.getLineItem(
										...
										function() {
											mw
													.log('PlayerControlBuilder::SwitchSourceMenu: '
															+ source.getSrc());
											// update menu selecting parent li
											// siblings
											...
											// TODO this logic should be in
											// mw.EmbedPlayer
											embedPlayer.mediaElement
													.setSource(source);
											if (!_this.embedPlayer.isStopped()) {
												// Get the exact play time from
												// the video element ( instead
												// of parent embed Player )
												var oldMediaTime = _this.embedPlayer
														.getPlayerElement().currentTime;
			...
			$.each(this.embedPlayer.mediaElement.getPlayableSources(),
					function(sourceIndex, source) {
						// Output the player select code:
						var supportingPlayers = mw.EmbedTypes.getMediaPlayers()
								.getMIMETypePlayers(source.getMIMEType());
						for ( var i = 0; i < supportingPlayers.length; i++) {
		 ...
		 * 
		 * @param {String}
		 *            component_id Component key to grab html output
		 */
		 ...
		 * 
		 * @param {String}
		 *            component_id Component key to grab height
		 ...
		 * 
		 * @param {String}
		 *            component_id Component key to grab width
		...
		/**
		 * Components Object Take in the embedPlayer and return some html for
		 * the given component.
		 * 
		 * components can be overwritten by skin javascript
		 * 
		 * Component JSON structure is as follows: 'o' Function to return a
		 * binded jQuery object ( accepts the ctrlObject as a parameter ) 'w'
		 * The width of the component 'h' The height of the component ( if
		 * height is undefined the height of the control bar is used )
		 */
		components : {
			/**
			...
			'attributionButton' : {
				...
				'o' : function(ctrlObj) {
					var buttonConfig = mw
							.getConfig('EmbedPlayer.AttributionButton');
					// Check for source ( by configuration convention this is a
					// 16x16 image
					...
					// update the configured size of the attribution button if
					// we have a specific width configured
			...
			'fullscreen' : {
				...
				'o' : function(ctrlObj) {
					var $btn = $('<div />')
							.attr('title',
									gM('mwe-embedplayer-player_fullscreen'))
							.addClass(
							...
							// Fullscreen binding:
							.buttonHover();
					// Link out to another window if iPad 3x ( broken iframe
					// resize )
					if ((mw.getConfig('EmbedPlayer.IsIframeServer') && mw
							.isIpad3())
							|| mw.getConfig("EmbedPlayer.NewWindowFullscreen")
							|| (mw.getConfig('EmbedPlayer.IsIframeServer') && mw
									.getConfig('EmbedPlayer.EnableIframeApi') === false)) {
						// Get the iframe url:
						var url = ctrlObj.embedPlayer.getIframeSourceUrl();
						// Change button into new window ( of the same url as
						// the iframe ) :
						return $('<a />').attr({
							'href' : url,
							'target' : '_new'
								...
								function() {
									...
									mw.setConfig(
											'EmbedPlayer.IsFullscreenIframe',
											true);
									// add a seek offset:
									mw.setConfig(
											'EmbedPlayer.IframeCurrentTime',
											ctrlObj.embedPlayer.currentTime);
									// add play state:
									mw.setConfig('EmbedPlayer.IframeIsPlaying',
											ctrlObj.embedPlayer.isPlaying());
									...
									var newwin = window.open(url,
											...
											'width=' + screen.width
													...
													+ ', top=0, left=0'
													+ ', fullscreen=yes');
									// if for some reason we could not open the
									// window run the href link:
									if (newwin === null) {
										return true;
									}
									if (window.focus) {
			...
			'volumeControl' : {
				'w' : 28,
				'o' : function(ctrlObj) {
					mw.log('PlayerControlBuilder::Set up volume control for: '
							+ ctrlObj.embedPlayer.id);
					var $volumeOut = $('<span />');
					...
					$volumeOut
							.append($('<div />')
									.attr(
											'title',
											gM('mwe-embedplayer-volume_control'))
									.addClass(
			...
			'sourceSwitch' : {
				'w' : 70,
				'o' : function(ctrlObj) {
					// Stream switching widget ( display the current selected
					// stream text )
					return $('<div />')
							.addClass('ui-widget source-switch')
							.append(
									ctrlObj.embedPlayer.mediaElement.selectedSource
											.getShortTitle())
							.menu(
									{
										'content' : ctrlObj
												.getSwitchSourceMenu(),
										'zindex' : mw
												.getConfig('EmbedPlayer.FullScreenZIndex') + 2,
										'width' : 115,
										'positionOpts' : {
											'posY' : 'top',
											'directionV' : 'up',
			...
			'timeDisplay' : {
				...
				'o' : function(ctrlObj) {
					return $('<div />').addClass("ui-widget time-disp").append(
				...
				'o' : function(ctrlObj) {
					...
					var sliderConfig = {
						...
						start : function(event, ui) {
							var id = (embedPlayer.pc != null) ? embedPlayer.pc.pp.id
									: embedPlayer.id;
							...
							embedPlayer.startTimeSec = (embedPlayer.instanceOf == 'mvPlayList') ? 0
									: mw.npt2seconds(embedPlayer.getTimeRange()
											.split('/')[0]);
						},
						slide : function(event, ui) {
							var perc = ui.value / 1000;
							embedPlayer.jumpTime = mw
									.seconds2npt(parseFloat(parseFloat(embedPlayer
											.getDuration())
											* perc)
											+ embedPlayer.startTimeSec);
							// mw.log('perc:' + perc + ' * ' +
							// embedPlayer.getDuration() + ' jt:'+
							// this.jumpTime);
							if (_this.longTimeDisp) {
								ctrlObj.setStatus(gM('mwe-embedplayer-seek_to',
										embedPlayer.jumpTime));
							} else {
						...
						change : function(event, ui) {
							// Only run the onChange event if done by a user
							// slide
							...
							if (embedPlayer.userSlide) {
								...
								var perc = ui.value / 1000;
								// set seek time (in case we have to do a url
								// seek)
								embedPlayer.seekTimeSec = mw.npt2seconds(
										embedPlayer.jumpTime, true);
								mw.log('PlayerControlBuilder:: seek to: '
										+ embedPlayer.jumpTime + ' perc:'
										+ perc + ' sts:'
										+ embedPlayer.seekTimeSec);
								ctrlObj
										.setStatus(gM('mwe-embedplayer-seeking'));
					...
					// TODO this will move into the disableSeekBar binding in
					// the new theme framework
					ctrlObj.disableSeekBar = function() {
						var $playHead = ctrlObj.embedPlayer.$interface
								.find(".play_head");
						if ($playHead.length) {
							...
							.css(
									{
										...
										"left" : '33px',
										"right" : ((embedPlayer
												.getPlayerWidth() - ctrlObj.available_width))
												+ 'px'
									})
							// Playhead binding
							.slider(sliderConfig);
					...
					$playHead.find('.ui-slider-range')
							.addClass('ui-corner-all').css('z-index', 2);
