	...
	$.fn.embedPlayer = function( attributes, callback ) {
		...
		mw.setupUserConfig( function() {
			...
			$j( playerSelect ).each( function( index, playerElement) {		
				...
				// if the target is not "video" or "audio" tag set background image
				// while we load sources
				var tagName = $j(playerElement).get(0).nodeName.toLowerCase();											
				if( tagName != 'video' && tagName != 'audio' && attributes.poster ){
					$j(playerElement).css("background-image", 'url(' + attributes.poster + ')' ); 
				}	
				// update position so spinner goes ontop
				$j(playerElement).css("position", 'absolute' );
...
EmbedPlayerManager.prototype = {
	...
	*/
	addElement: function( playerElement,  attributes ) {	
		...
		// Merge in any custom attributes to the player element:
		for( var i in attributes ){
			playerElement[i] = attributes[i];
		}
	...
	*/
	checkPlayerSources: function() {
		...
		var finishCheckPlayerSources = function(){
			// Run embedPlayer CheckPlayerSources sources hook
