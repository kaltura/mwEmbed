	...
	autoSelectSourceExecute: function () {
		...
		if ( mw.isMobileDevice() && namedSourceSet.h264 && namedSourceSet.h264.length ) {
			...
			$.each( namedSourceSet.h264, function ( inx, source ) {
				...
				if ( parseInt( source.width, 10 ) < parseInt( minSize, 10 ) && parseInt( source.width, 10 ) !== 0 ) {
		...
		function sizeFilter( inx, source ) {
			if ( parseInt( source.width, 10 ) && displayWidth ) {
			...
			for ( i = 0; i < codecPref.length; i++ ) {
				...
				if ( namedSourceSet[ codec ].length === 1 ) {
					...
					if ( this.parentEmbedId ) {
						...
						$.each( namedSourceSet[ codec ], sizeFilter );
					}
