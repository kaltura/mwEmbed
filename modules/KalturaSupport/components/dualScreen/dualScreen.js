	...
	mw.PluginManager.add('dualScreen', mw.KBaseComponent.extend({
		...
		initFSM: function () {
			...
			var fsmTransitionHandlers = function (transitionFrom, transitionTo) {
				...
				_this.getPlayer().triggerHelper('preDualScreenTransition', [
				...
				function transitionendHandler(e) {
					if (transitionHandlerSet) {
						...
						_this.getPlayer().triggerHelper('postDualScreenTransition', [
							[transitionFrom, transitionTo]
						]);
					}
				}
			...
			monitor
				...
				.on('mousemove touchstart', function (e) {
					if (!this.dragging) {
						...
						cornerHandleVisibleTimoutId = setTimeout(function () {
							_this.hideResizeHandlers(monitorRef);
						}, _this.getConfig('menuFadeout'))
					}
				});
