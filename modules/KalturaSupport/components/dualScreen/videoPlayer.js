(function (mw, $, kWidgetSupport, gM) {
  ...
  mw.dualScreen.videoPlayer = mw.KBaseComponent.extend({
    ...
    initNativePlayer: function () {
      ...
      this.$el.get(0).getCurrentTime = function () {
        return _this.$el.get(0).currentTime;
      };
    ...
    initKPlayer: function () {
      ...
      new mw.PlayerElementFlash('secondScreen', 'vidSibling_obj', fv, _this, function () {
        var bindEventMap = {
    ...
    sync: function () {
      if (!this.videoSync) {
        this.videoSync = new mw.dualScreen.videoSync(this.getPlayer(), $.noop, 'videoSync');
...
})(window.mw, window.jQuery, window.kWidgetSupport, window.gM);