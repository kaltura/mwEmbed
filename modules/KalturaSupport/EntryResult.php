        ...
        if (isset($resultObject['entryCuePoints'])
            ...
            if ($countDiff != 0) {
                try {
                    ...
                    // retrieve the number of missing pages - for example: 700 / 500 = ceil(1.4) = 2 pages (500 from the first and 200 from the second)
                    ...
                    $extraPagesObject = $pagesMultiRequest->doQueue();
                } catch (Exception $e) {
                    // Update the Exception and pass it upward
                    ChromePhp::info("error error error");
                    throw new Exception(KALTURA_GENERIC_SERVER_ERROR . "\n" . $e->getMessage());
                    return array();
                }
                $mergedResults = $resultObject['entryCuePoints']->objects;
                for ($i = 0; $i < $missingPages; $i++) {
                    $requestedPage = $i + 2;
                    if (isset($extraPagesObject['entryCuePoints_page' . $requestedPage])
                        && is_object($extraPagesObject['entryCuePoints_page' . $requestedPage])
                        && count($extraPagesObject['entryCuePoints_page' . $requestedPage]) > 0
                    ) {
                        $mergedResults = array_merge($mergedResults, $extraPagesObject['entryCuePoints_page' . $requestedPage]->objects);
                    }
                }
                $resultObject['entryCuePoints'] = $mergedResults;
            } else {
