(function (mw, $) {
	...
	mw.PluginManager.add('webcastPolls', mw.KBasePlugin.extend({
		...
		filterPollResultsCuePoints: function (context) {
			...
			if (_this.pushCuePointsManager && pollData.showAnswers) {
				...
				for (var i = 0; i < _this.pollsCuePoints.length; i++) {
					if (_this.pollsCuePoints[i].tags == "poll-results" && this.getPollId(_this.pollsCuePoints[i]) == pollId) {
		...
		getPollId: function (cuePoint) {
			try {
				var cuePointPartnerData = this.cuePointPartnerData(cuePoint);
				var cuePointId = cuePointPartnerData.pollId;
				if (cuePointId) {
					return cuePointId;
				}
			} catch (e) {
		...
		cuePointPartnerData: function (cuePoint) {
			if (cuePoint && cuePoint.partnerData) {
				return JSON.parse(cuePoint.partnerData);
			}
			return null;
		},
		executeCuePointReached: function (cuePoint) {
			//TODO handle catch-up when
			var cuePointId = this.getPollId(cuePoint);
			if (cuePoint.tags == "poll-results") {
				//if catch-up with a poll-result we need to show the previous poll-show
				for (var i = this.pollsCuePoints.length - 1; i >= 0; --i) {
					var localCuePoint = this.pollsCuePoints[i];
					var localCuepointPollId = this.getPollId(localCuePoint);
					var localCuepointPartnerData = this.cuePointPartnerData(localCuePoint);

					if (localCuepointPollId == cuePointId && localCuepointPartnerData && localCuePoint.tags != "poll-results") {
						//found the previous active CP of this poll
						this.latestCuePointReached = cuePoint;
						this.handleStateCuePoints({cuepointsArgs: {cuePoints: [localCuePoint]}});
						//todo - activate later the results
						return;
						break;
					}
				}
		...
		voteInProgress: function () {
			var _this = this;
			// TODO - re-hook when service of checkMyVote will be ready
			// TODO - for now simulate not-in-progress
			return false;
