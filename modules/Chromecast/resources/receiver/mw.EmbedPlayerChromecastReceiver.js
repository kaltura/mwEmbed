    ...
    mw.EmbedPlayerChromecastReceiver = {
        ...
        parseTracks: function () {
            ...
            for ( var trackId = 0; trackId < streamCount; trackId++ ) {
                ...
                } else if ( info.mimeType.indexOf( 'video' ) === 0  && !info.codecs.startsWith("mp4a") && !info.language) {
                    trackType = top.cast.receiver.media.TrackType.VIDEO;
                    if ( isActive ) {
                        this.videoStreamIndex = trackId;
                    }
                } else if ( info.mimeType.indexOf( 'audio' ) === 0  || (info.mimeType == "video/mp2t" && info.codecs.startsWith("mp4a"))) {
