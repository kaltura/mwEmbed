...
( function( mw, $ ) {"use strict";
	...
	mw.TimedText.prototype = {
		 ...
		 */
		buildMenuCounter: 0, // for counter timeouts
		buildMenu: function( autoShow ) {
			var _this = this;
			var $menuButton = _this.embedPlayer.$interface.find( '.timed-text' );
			if ($menuButton.length == 0 && _this.buildMenuCounter < 10){
				_this.buildMenuCounter ++;
				setTimeout(function(){
					_this.buildMenu(autoShow);
				},500);
			}else{
				var embedPlayer = this.embedPlayer;
		...
		monitor: function( ) {
			...
			if (this.enabledSources.length > 0){
				window.enabledSource = this.enabledSources[0];
			}
			if( window.enabledSource ) {
				this.updateSourceDisplay( window.enabledSource, currentTime );
			}
		},
